/**
 * usePrintPdf — genera e stampa il piano alimentare settimanale come PDF.
 *
 * Approccio: zero librerie esterne.
 * 1. Costruisce una stringa HTML con il layout del documento.
 * 2. La inietta in un <div> temporaneo nel <body>.
 * 3. Aggiunge un <style> che, a stampa, nasconde tutto tranne il div.
 * 4. Chiama window.print() — il browser apre il dialog "Salva come PDF".
 * 5. Sull'evento `afterprint` rimuove tutto dal DOM.
 *
 * Regole di filtro:
 * - I pasti senza alimenti vengono omessi.
 * - I giorni in cui TUTTI i pasti sono vuoti vengono omessi.
 */

import { DAY_KEYS, MEAL_TYPE_COLOR } from '@/data/defaults'
import type { DayPlan, Meal, NutritionTotals } from '@/types'
import { macrosForFood, round1, round0, sumTotals, emptyTotals, totalsForMeal, totalsForDay } from '@/utils/nutrition'
    ...
    .map((food) => {
      ...
      return `
        <tr>
          ...
          <td>${fmtKcal(m.kcal)}</td>
          <td>${fmt(m.proteine)}</td>
          <td>${fmt(m.carboidrati)}</td>
          <td>${fmt(m.grassi)}</td>
        </tr>`
    })
    .join('')
...
function buildDaySection(day: DayPlan): string {
  ...
  const totals: NutritionTotals = totalsForDay({
    ...day,
    meals: activeMeals
  })
...
// ── CSS inline del documento di stampa ─────────────────────────
...
const PDF_STYLES = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .meal-note {
    ...
    font-size: 9px; font-style: italic; color: #777;
    background: #fafaf9;
  ...
  .day-total-lbl {
    ...
    color: #3a7a56; margin-right: auto;
  }
...
// ── CSS iniettato nell'app per nasconderla durante la stampa ────
...
// ── Costruzione documento completo ──────────────────────────────
...
function buildDocument(
  ...
  const activeDays = days.filter((d) =>
    d.meals.some((m) => m.alimenti.length > 0)
  )

  if (activeDays.length === 0) {
    return `<p style="padding:40px 24px;font-family:sans-serif;color:#666;">
      Nessun pasto con alimenti da stampare.
    </p>`
  }
  ...
  const dataFormattata = patient.dataScheda
    ? new Date(patient.dataScheda).toLocaleDateString('it-IT', {
  ...
  return `
    ...
    <div class="pdf-patient">
      <div class="pf">
        <span class="pf-lbl">Paziente</span>
        <span class="pf-val">${patient.nome}</span>
      </div>
      <div class="pf">
        <span class="pf-lbl">Data scheda</span>
        <span class="pf-val">${dataFormattata}</span>
      </div>
      <div class="pf">
        <span class="pf-lbl">Obiettivo calorico</span>
        <span class="pf-val">${round0(patient.obiettivoKcal).toLocaleString('it-IT')} kcal / giorno</span>
      </div>
    </div>
...
export function usePrintPdf() {
  ...
  function printPdf(): void {
    ...
    // Override CSS per nascondere l'app durante la stampa
    const overrideStyle = document.createElement('style')
    overrideStyle.id = '__pdf_print_override__'
    overrideStyle.textContent = PRINT_OVERRIDE_STYLES
    document.head.appendChild(overrideStyle)

    // Mostra il contenitore (serve per il layout pre-stampa)
    container.style.display = 'block'

    // Imposta il titolo della pagina: il browser lo usa come nome file PDF
